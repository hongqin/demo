  ...
  tr = read.nexus( file  );
    plot( tr, main=paste("BG14052",file), show.node.label=T );
    plot( tr, main=paste("BG14052",file), type="u" );
    i;
  }

  t2nj = read.nexus( as.character(files[2]) );
  t3bs = read.nexus( as.character(files[3]) );

  d = dist.topo( t2nj, t3bs);
  if( d[1]== 0 ) { 
